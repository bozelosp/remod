...
function uniqueFileName(name, pending = [], excludeId = null) {
  ...
  while (occupied.has(fitFileName(stem, ` (${index}).swc`).toLowerCase())) index += 1;
  return fitFileName(stem, ` (${index}).swc`);
}
...
async function api(path, options = {}) {
  if (!path.startsWith("/api/")) throw new Error("Only local API paths are supported");
  const response = await fetch(path, {
    method: options.method || "GET",
    headers: options.body ? { "content-type": "application/json", "X-Remod-Request": "1" } : {},
...
function edgeSignature(morphology, proximal, distal) {
  const value = (number) => Number(number).toPrecision(17);
...
async function readFiles(fileList) {
  ...
  try {
    if (state.files.length + fileList.length > 128) throw new Error("A workspace supports up to 128 files");
    let bytes = state.files.reduce((sum, file) => sum + new TextEncoder().encode(file.content).length, 0);
    for (const file of [...fileList]) {
      safeFileName(file.name);
      if (!file.name.toLowerCase().endsWith(".swc")) throw new Error(`${file.name} is not an SWC file`);
      bytes += file.size;
      if (file.size > 16 * 1024 * 1024 || bytes > 64 * 1024 * 1024) throw new Error("Limit: 16 MiB per SWC and 64 MiB per workspace");
      const content = new TextDecoder("utf-8", { fatal: true }).decode(await file.arrayBuffer()); additions.push(createFile(uniqueFileName(file.name, additions), content));
    }
    checkWorkspaceCapacity(new Map(), null, additions);
  } catch (error) {
...
async function loadExample() {
  ...
  try {
    if (state.files.length >= 128) throw new Error("A workspace supports up to 128 files");
    ...
    checkWorkspaceCapacity(new Map(), null, [file]);
...
function checkWorkspaceCapacity(replacements = new Map(), preview = state.preview, additions = []) {
  const analyses = new Set(), sources = new Set(), encoder = new TextEncoder(); let currentBytes = 0;
  const retain = (item) => { if (item?.analysis) analyses.add(item.analysis); if (typeof item?.content === "string") sources.add(item.content); };
  for (const original of [...state.files, ...additions]) {
    const file = { ...original, ...replacements.get(original) };
    currentBytes += encoder.encode(file.content).length; sources.add(file.originalContent); retain(file); file.history.forEach(retain);
  }
  retain(preview);
  const samples = [...analyses].reduce((sum, analysis) => sum + analysis.morphology.counts.samples, 0);
  const bytes = [...sources].reduce((sum, source) => sum + encoder.encode(source).length, 0);
  if (samples > 250000) throw new Error("Workspace geometry exceeds 250,000 retained samples; remove a file or export and reload edited files");
  if (currentBytes > 64 * 1024 * 1024 || bytes > 128 * 1024 * 1024) throw new Error("Workspace text exceeds 64 MiB of current files or 128 MiB including originals, previews, and undo history; export and remove a file to continue");
}

function assignAnalysis(file, prepared) {
  checkWorkspaceCapacity(new Map([[file, prepared]]));
...
async function analyzeFiles(files = state.files, { atomic = false } = {}) {
  ...
  async function worker() {
    while (cursor < pending.length) {
      ...
      try {
        ...
        if (atomic) { checkWorkspaceCapacity(new Map([...staged, [file, prepared]])); staged.push([file, prepared]); } else assignAnalysis(file, prepared);
      } catch (error) {
        if (generation !== state.analysisGeneration) return;
  ...
  if (atomic && !errors.length) { try { checkWorkspaceCapacity(new Map(staged)); } catch (error) { errors.push(error.message); } }
  ...
  if (atomic) { staged.forEach(([file, prepared]) => Object.assign(file, prepared, { status: "ready", error: null })); state.shollStep = step; }
...
function explicitSeed() {
  const field = $("#seed"); if (field.value.trim() !== "") return field.value; const values = new Uint32Array(1); crypto.getRandomValues(values); const seed = values[0] & 0x7fffffff; field.value = String(seed); return field.value;
}
function remodelOptions() {
  ...
  return {
    who, action, amount: ["remove", "none"].includes(action) ? null : $("#amount").value,
    random_ratio: who.startsWith("random_") ? $("#randomRatio").value : 0,
    manual_segments: who === "manual" ? [...state.selectedSegments].sort((a, b) => a - b).join(",") : "",
    extent_unit: $("#extentUnit").value, radius_change: action === "remove" || $("#radiusChange").value.trim() === "" ? null : $("#radiusChange").value,
    radius_unit: $("#radiusUnit").value, seed: stochastic ? explicitSeed() : $("#seed").value.trim() === "" ? null : $("#seed").value,
  };
}
...
async function generatePreview(event) {
  ...
  try {
    ...
    safeFileName(result.name);
    const prepared = { fileId: file.id, sourceContent, name: result.name, content: result.content, targets: result.targets, selector: result.selector, analysisId: result.analysis_id, analysis: analysisFromResponse(result), changes: result.changes, warnings: result.warnings, impact: result.impact, elapsed: result.elapsed_ms, options, label: operationLabel(options, result.targets.length) };
    checkWorkspaceCapacity(new Map(), prepared); state.preview = prepared;
...
async function applyPreview() {
  ...
  if (file.history.length >= 20) { showToast("The 20-edit undo limit is reached. Export this morphology and load it as a new file to continue; no history was discarded.", true); return; }
  const prior = { name: file.name, content: file.content, analysis: file.analysis, analysisId: file.analysisId, analysisStep: file.analysisStep, elapsed: file.elapsed, cached: file.cached, operation: preview.label, targets: [...preview.targets], options: preview.options };
  let next;
  try {
    next = { name: uniqueFileName(preview.name, [], file.id), content: preview.content, analysis: preview.analysis, analysisId: preview.analysisId, analysisStep: currentStep(), elapsed: preview.elapsed, cached: true, history: [...file.history, prior] };
    checkWorkspaceCapacity(new Map([[file, next]]), null);
  } catch (error) { showToast(error.message, true); return; }
  Object.assign(file, next); state.preview = null; state.selectedSegments.clear(); state.hoveredSegment = null;
...
function safeFileName(name) { if (typeof name !== "string" || !name || new TextEncoder().encode(name).length > 240 || /[\\/:\p{Cc}\p{Cf}\p{Cs}\p{Zl}\p{Zp}]/u.test(name) || [".", ".."].includes(name)) throw new Error("Use a plain filename of at most 240 UTF-8 bytes, without paths or control characters"); return name; }
function fitFileName(stem, suffix) {
  const characters = [...stem], encoder = new TextEncoder();
  while (characters.length && encoder.encode(characters.join("") + suffix).length > 240) characters.pop();
  return safeFileName(characters.join("") + suffix);
}
function exportFileName(name, suffix) { safeFileName(name); return fitFileName(name.replace(/\.swc$/i, ""), suffix); }
function download(name, content, type) { safeFileName(name); const blob = new Blob([content], { type }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = name; anchor.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
function csvCell(value) { let text = String(value ?? ""); if (typeof value === "string" && (/^\s*[=+@-]/u.test(text) || /^[\t\r\n]/.test(text))) text = `'${text}`; return /[",\r\n]/.test(text) ? `"${text.replaceAll('"', '""')}"` : text; }
...
function downloadActiveJson() { const file = activeFile(); if (file?.analysis) download(exportFileName(file.name, "_statistics.json"), `${JSON.stringify(file.analysis.statistics, null, 2)}\n`, "application/json"); }
...
function downloadScalarCsv() { const file = activeFile(); if (file?.analysis) download(exportFileName(file.name, "_scalar_metrics.csv"), scalarCsvContent(), "text/csv"); }
...
function bindEvents() {
  ...
  $("#downloadSwc").addEventListener("click", downloadActiveSwc); $("#downloadJson").addEventListener("click", downloadActiveJson); $("#downloadMetricsCsv").addEventListener("click", downloadScalarCsv); $("#downloadShollCsv").addEventListener("click", () => { const file = activeFile(); if (!file) return; const generic = !file.analysis.morphology.root.is_soma || file.analysis.morphology.counts.dendrites === 0, key = generic ? `radial_all_arbor_${$("#shollMetric").value}` : `sholl_${$("#shollRegion").value}_${$("#shollMetric").value}`; download(exportFileName(file.name, `_${key}.csv`), distributionCsv(key), "text/csv"); }); $("#downloadBranchCsv").addEventListener("click", () => { const file = activeFile(), key = branchKey(); if (file) download(exportFileName(file.name, `_${key}.csv`), distributionCsv(key), "text/csv"); });
