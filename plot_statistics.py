...
def _aggregate_source(path: Path) -> MetricSource:
    ...
    unavailable = summary.get("capability_counts", {}).get("dendrite_specific_morphometrics") == 0
    if unavailable:
        note += "; no eligible dendritic measurements"
    return MetricSource(scalars, distributions, True, note, unavailable)
...
def _save(plt, fig, path: Path, title: str) -> None:
    ...
    plt.close(fig)
    _write_new(path, rendered.getvalue().decode("utf-8"), overwrite=True)
