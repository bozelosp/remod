...
from remod.model import (
    MAX_SWC_BYTES, MAX_NODES, MAX_LINE_CHARS, MAX_COMMENT_BYTES,
    _finite_token, _integral_token, _plain_text,
)

MAX_TOPOLOGY_LINKS = 1_000_000
...
def parse_swc_lines(swc_lines: Iterable[str]) -> Tuple[List[str], SampleMap]:
    ...
    for line_number, raw_line in enumerate(swc_lines, start=1):
        ...
        byte_count += len(raw_line.encode("utf-8")) + 1
        if byte_count > MAX_SWC_BYTES:
            raise ValueError(f"SWC input exceeds {MAX_SWC_BYTES} bytes")
        line = raw_line.strip()
        if not line:
            continue
        if line.startswith("#"):
            comment_bytes += len(raw_line.encode("utf-8")) + 1
        ...
        try:
            sample_id = _integral_token(fields[0], line_number=line_number, field_name="id")
            sample_type = _integral_token(fields[1], line_number=line_number, field_name="kind")
            x, y, z, radius = map(_finite_token, fields[2:6])
            parent_id = _integral_token(fields[6], line_number=line_number, field_name="parent")
        except ValueError as exc:
            raise ValueError(f"line {line_number}: invalid SWC value ({exc})") from exc
...
def validate_samples(samples: SampleMap) -> None:
    ...
    for key, sample in samples.items():
        ...
        if any(not -(1 << 63) <= int(sample[index]) < (1 << 63) for index in (0, 1, 6)):
            raise ValueError("SWC integer fields must fit signed 64-bit integers")
...
def segment_roots(
    ...
    non_soma_counts = {
        parent: sum(int(samples[child][1]) != 1 for child in child_ids)
    ...
    for sample_id, sample in samples.items():
        ...
        if (
            ...
            or
            int(parent[1]) == 1
            or non_soma_counts[parent_id] != 1
...
def paths_to_root(
    ...
    for root in sorted(roots):
        ...
        while parent_id not in anchor_ids and parent_id != -1:
            ...
            links += 1
            if links > MAX_TOPOLOGY_LINKS:
                raise ValueError(f"Studio topology exceeds {MAX_TOPOLOGY_LINKS} ancestor links")
    ...
    for root in dendrite_roots:
        ...
        for proximal, distal in zip(points, points[1:]):
            ...
            if edge_length:
                ...
                edge_area = 2.0 * pi * radius * edge_length
                edge_volume = pi * radius * radius * edge_length
                if edge_area == 0.0 or edge_volume == 0.0:
                    raise ValueError("positive cylindrical geometry underflows binary64")
                area += edge_area
                volume += edge_volume
...
def build_morphology(
    ...
    for comment in comments:
        ...
        if not comment.lstrip().startswith("#") or len(comment) > MAX_LINE_CHARS:
            raise ValueError("comments must be bounded single-line SWC comments")
        comment_bytes += len(comment.encode("utf-8")) + 1
        if comment_bytes > MAX_COMMENT_BYTES:
            raise ValueError(f"comments exceed {MAX_COMMENT_BYTES} bytes")
...
def parse_swc_text(text: str) -> ParsedMorphology:
    ...
    if not isinstance(text, str):
        raise ValueError("SWC input must be text")
    if len(text) > MAX_SWC_BYTES or len(text.encode("utf-8")) > MAX_SWC_BYTES:
        raise ValueError(f"SWC input exceeds {MAX_SWC_BYTES} bytes")
    comments, samples = parse_swc_lines(StringIO(text, newline=None))
