"""Geometry-preserving arbor remodeling actions."""
...
def create_points(
    ...
    soma_origin: Iterable[float] | None,
    branch_option: int,
    rng: random.Random | None = None,
    *,
    plane_axis: int | None = None,
) -> List[List[float]]:
    """Create one or two representable points on a local direction cone.

    A supplied soma origin enforces the paper's soma-outward constraint. If no
    soma exists, local forward direction is used without inventing a biological
    origin. For planar reconstructions, ``plane_axis`` keeps generated points
    in the recorded plane.
    """
    ...
    origin = (
        None
        if soma_origin is None
        else np.asarray(list(soma_origin), dtype=float)
    )
    if start.shape != (3,) or end.shape != (3,) or (
        origin is not None and origin.shape != (3,)
    ):
        raise ValueError("growth points and optional soma origin must be 3D")
    finite_values = [*start, *end] + ([] if origin is None else [*origin])
    if not np.isfinite(finite_values).all():
        raise ValueError("growth points and optional soma origin must be finite")
    if plane_axis not in {None, 0, 1, 2}:
        raise ValueError("plane axis must be 0, 1, 2, or none")
    ...
    if plane_axis is None:
        ...
        normal = np.eye(3, dtype=float)[plane_axis]
        in_plane = axis - float(np.dot(axis, normal)) * normal
        axis = _unit(in_plane, "in-plane parent segment")
        perpendicular = _unit(np.cross(normal, axis), "in-plane perpendicular")
        tilted = axis
    current_radius = None if origin is None else _radial_distance(end, origin)
    ...
    for _attempt in range(MAX_DIRECTION_ATTEMPTS):
        if plane_axis is None:
            ...
            directions = [
                cos(deflection) * axis + sign * sin(deflection) * perpendicular
            ]
            if branch_option == 2:
                directions.append(
                    cos(deflection) * axis - sign * sin(deflection) * perpendicular
        ...
        if origin is not None and any(
            _radial_distance(candidate, origin) < float(current_radius)
    ...
    if origin is not None:
        raise ValueError(
            "requested growth cannot be represented outward from the soma within "
            "the local direction cone"
        )
    raise ValueError("requested growth cannot be represented within the local direction cone")
    ...
    for child in descendants.get(dendrite, []):
        for sample in dendrite_samples.get(child, []):
            ...
            if sample_id in moved:
                continue
            before = np.asarray(sample[2:5], dtype=float)
            after = before + vector
            if not np.isfinite(after).all() or any(
                delta != 0 and (new == old or (new > old) != (delta > 0))
                for old, new, delta in zip(before, after, vector)
            ):
                raise ValueError("descendant translation collapses a nonzero component")
            sample[2:5] = after.tolist()
            moved.add(sample_id)
...
def shrink(
    ...
    for root in list(target_dendrites):
        if root not in dendrite_samples:
            raise ValueError(f"unknown arbor segment {root}")
        ...
        lookup.update((int(sample[0]), sample) for sample in dendrite_samples[root])
...
def extend(
    ...
    rng: random.Random | None = None,
    *,
    growth_origin: Iterable[float] | None = None,
    plane_axis: int | None = None,
) -> None:
    ...
    soma_origin = (
        None
        if growth_origin is None
        else np.asarray(list(growth_origin), dtype=float)
    )
    for root in list(target_dendrites):
        ...
        lookup.update((int(sample[0]), sample) for sample in rows)
    ...
    *,
    ...
    for root in list(target_dendrites):
        ...
        for point in first_points:
            ...
            lookup.update((int(sample[0]), sample) for sample in daughter)
...
def radius_change(
    ...
    """Change selected segment radii by a percentage or native coordinate units."""

    value = float(change)
    if not np.isfinite(value):
        raise ValueError("radius change must be finite")
    if unit not in {"percent", "units"}:
...
def scale(
    ...
    if unknown:
        raise ValueError(f"unknown arbor segment(s): {unknown}")
    ...
    for children in descendants.values():
        for child in children:
            if child in counts:
                counts[child] += 1
    lookup = _sample_lookup(soma_samples, dendrite_samples)
    for root in sorted(selected, key=lambda item: (counts[item], item)):
...
def execute_action(
    ...
    *,
    ...
    if action in {"shrink", "extend", "scale"}:
        work = 0
        for root in targets:
            work += len(dendrite_samples[root])
            work += sum(len(dendrite_samples[child]) for child in descendants.get(root, []))
            if work > MAX_EDIT_WORK:
                raise ValueError(f"edit exceeds {MAX_EDIT_WORK} sample visits; use a smaller selection")
    if action in {"extend", "branch"}:
        # Each sampled step is at least min(LENGTHS), except the final cut.
        # Reserve for every selected path before any geometry is generated.
        existing = len(_sample_lookup(soma_samples, dendrite_samples))
        daughters = 2 if action == "branch" else 1
        budget = existing
        for root in targets:
            extent = _growth_distance(lengths[root], amount, extent_unit)
            _validate_growth_target(extent)
            budget += daughters * (ceil(extent / min(LENGTHS)) + 1)
            if budget > MAX_NODES:
                raise ValueError(f"requested growth exceeds the {MAX_NODES}-sample generation budget")
