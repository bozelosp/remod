...
from remod_engine import RemodelRequest, remodel_text, validate_file_name
...
def analyze_main(argv: list[str] | None = None) -> int:
    ...
    write_json(statistics_dir / "results.json", results, overwrite=True)
    write_json(statistics_dir / "summary.json", summarize_statistics(results), overwrite=True)
...
def edit_main(argv: list[str] | None = None) -> int:
    ...
    request = RemodelRequest(
        ...
        manual_segments=options.manual_segments,
    ...
    result = remodel_text(_read_text(source_path), request)
