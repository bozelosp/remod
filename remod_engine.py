...
from remod.model import MAX_NODES, MAX_LINE_CHARS, _plain_text
...
def _analysis_id(source_id: str, sholl_step: float) -> str:
    digest = sha256()
    digest.update(b"remod-analysis-v4\0")
...
class AnalysisCache:
    ...
    def _bound_bytes(self) -> None:
        # Count retained Python objects once, including shared parsed sources.
        def retained_size() -> int:
            pending = [self._entries, self._sources, self._statistics]
            seen = set()
            total = 0
            while pending:
                item = pending.pop()
            ...
            return total
        self.bytes = retained_size()
        for collection in (self._entries, self._sources, self._statistics):
            while collection and self.bytes > self.max_bytes:
...
def validate_remodel_request(request: RemodelRequest) -> None:
    ...
    if request.seed is not None and (
        type(request.seed) is not int or not -(2**53 - 1) <= request.seed <= 2**53 - 1
    ):
        raise ValueError("seed must be an exactly representable integer")
    if not isinstance(request.manual_segments, str) or len(request.manual_segments) > MAX_NODES * 21:
        raise ValueError("manual selection is too large")
    selectors = {
        "all_arbors",
        "all_terminal_arbors",
        "all_axons",
    ...
    if request.who not in selectors:
        raise ValueError(f"unknown arbor selector: {request.who}")
    ...
    if request.extent_unit not in {"percent", "units"}:
        raise ValueError("extent unit must be 'percent' or 'units'")
    if (
    ...
    if request.radius_unit not in {"percent", "units"}:
        raise ValueError("radius unit must be 'percent' or 'units'")
...
def validate_file_name(name: str) -> str:
    if not isinstance(name, str) or not name or len(name.encode("utf-8")) > 240:
        raise ValueError("filename must contain 1–240 UTF-8 bytes")
    _plain_text(name, "filename", allow_tab=False)
    if name in {".", ".."} or any(char in name for char in "/\\:"):
        raise ValueError("filename must be a plain basename, not a path")
    return name
...
def select_dendrites(
    ...
    fixed = {
        "all_arbors": (parsed.arbor_roots, "all arbor"),
        "all_terminal_arbors": (parsed.arbor_terminal, "all terminal arbor"),
        "all_axons": (parsed.axon, "all axonal"),
        "all_unspecified_neurites": (
            parsed.unspecified,
            "all unspecified-neurite",
        ),
        "all_glial_processes": (parsed.glia, "all glial-process"),
    ...
    random_groups = {
        "random_all_arbors": (parsed.arbor_terminal, "terminal arbor"),
    ...
    if request.who == "manual":
        targets = _manual_selection(request.manual_segments)
        label = "manual"
    ...
    known = set(parsed.arbor_roots)
    invalid = sorted(set(targets) - known)
    if invalid:
        raise ValueError(f"unknown arbor segment ID(s): {invalid}")
    if not targets:
        raise ValueError("the requested selection contains no matching arbor segments")
...
def _edit_header(
    ...
    for root in targets:
        ...
        if len(line) + len(separator) + len(token) > MAX_LINE_CHARS:
            target_lines.append(line)
            line, separator = "# segment_ids: ", ""
...
def remodel_text(
    ...
    *,
    ...
    if request.action in {"extend", "branch"}:
        unsupported = sorted(
            root for root in targets if int(working.segments[root][0][1]) not in {3, 4}
        )
        if unsupported:
            types = sorted({int(working.segments[root][0][1]) for root in unsupported})
            raise ValueError(
                "extend and branch use REMOD's dendrite-derived growth model and "
                f"are not scientifically assigned to SWC type(s) {types}; target "
                f"segment(s) {unsupported} can still be shrunk, removed, scaled, "
                "or radius-edited"
            )
        if working.spatial_dimension < 2:
            raise ValueError(
                "extend and branch require at least a 2D reconstruction to define "
                "a scientifically interpretable growth direction"
            )
    growth_origin = (
        tuple(float(value) for value in working.root_sample[2:5])
        if working.root_is_soma
        else None
    )
    plane_axis = (
...
def morphology_payload(parsed: ParsedMorphology) -> dict[str, object]:
    ...
    if any(abs(int(sample[index])) > 2**53 - 1
           for sample in parsed.samples.values() for index in (0, 1, 6)):
        raise ValueError("Studio requires exactly representable JavaScript integer IDs and types (at most 2^53−1)")
    sample_segments = {
        int(sample[0]): root
        for root in parsed.arbor_roots
    ...
    terminal = set(parsed.arbor_terminal)
    segments = [
        [
            root,
            ...
            parsed.root_paths[root][1] if len(parsed.root_paths[root]) > 1 else None,
    ...
    return {
        ...
        "bounds": bounds,
        "root": {
            "sample_id": int(parsed.root_sample[0]),
            "type": int(parsed.root_sample[1]),
            "is_soma": parsed.root_is_soma,
        },
        ...
        "counts": {
            "samples": len(samples),
            "segments": len(parsed.arbor_roots),
            "terminals": len(parsed.arbor_terminal),
            "branchpoints": len(parsed.arbor_branch_points),
            "dendrites": len(parsed.dendrite_roots),
            "dendritic_terminals": len(parsed.all_terminal),
            "dendritic_branchpoints": len(parsed.branch_points),
            "basal": len(parsed.basal),
            "apical": len(parsed.apical),
            "axon": len(parsed.axon),
            "undefined": len(parsed.undefined),
            "custom": len(parsed.custom),
            "unspecified": len(parsed.unspecified),
            "glia": len(parsed.glia),
