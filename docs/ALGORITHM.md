...
is recorded by its distal SWC sample. This is the compartment convention used
by the publication-era REMOD implementation. For the bundled NMO_01999 fixture,
the resulting centerline length, lateral area, and volume reproduce the values
in its [NeuroMorpho.Org morphometry
record](https://neuromorpho.org/api/morphometry/id/1999). This is a
fixture-level check, not validation for every SWC file. The soma radius is not
used for a soma-to-neurite edge. A zero-length edge contributes zero length,
area, and volume.
...
Generation is limited to 100,000 new samples per path. A request beyond that
bound, or below the coordinate precision at the selected tip, is rejected
without writing an output file.

## Serialization

Edited output is parsed and validated before it is written. Samples are then
renumbered contiguously in deterministic parent-before-child order. Output
headers generated by REMOD record the source file name, selector, segment
identifiers, action, amounts, and seed without adding local paths or timestamps.
Existing source comments are preserved unchanged.
