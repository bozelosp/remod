# Studio scientific contract

This document defines `remod_ui.py`, `remod_cli.py`, and their shared Studio
engine. The minimal `remod` kernel has a different contract in
[ALGORITHM.md](ALGORITHM.md). The scientific
...
Inputs use bounded ASCII decimal tokens, strict UTF-8, and LF/CRLF/CR lines.
Nonzero values rounded to zero, control/format characters, oversized records,
and unrepresentable geometry are rejected, never repaired. Integer fields fit
signed 64-bit; browser geometry additionally requires JavaScript-exact integers
(up to 2^53 - 1 in magnitude). Operational limits are in [DEFENSE.md](DEFENSE.md).
...
The radial implementation shares the kernel's exact rational contact solver,
including endpoint, tangent, and collapsed-contact checks. Shell sums use
`math.fsum`. Replacing the former binary64 quadratic computation may change
last-bit shell lengths, not the definitions above. Requests are limited to
10,000 bins and 200,000 weighted exact-arithmetic work units per profile call.
Increase the radial step if the requested discretization exceeds capacity.
...
it does not repeat random selection or geometry generation. An applied edit
retains its exact prior state for undo. Capacity failures cannot consume an undo
slot or partially replace a workspace analysis. Studio supplies an
...
50,000 samples in the entire resulting morphology. Before generation, each
selected daughter reserves `ceil(requested length / minimum table step) + 1`
samples. This conservative worst-case reservation bounds allocation without
changing the random draw sequence. Topology and edit-work limits also apply.
A request beyond capacity, below coordinate precision, or one whose descendant
translation collapses a nonzero vector is rejected without publishing output.
...
destination is rejected unless `--force` is supplied. Both modes use the shared
verified atomic writer; force may replace only a regular, single-link file,
never an input alias. Browser downloads use the browser's save behavior.
...
source comments are preserved unchanged. Long target lists span multiple
`# segment_ids:` lines without dropping IDs. Browser filenames are plain names
of at most 240 UTF-8 bytes; automatically generated duplicate, edit, and export
names shorten only the stem when a suffix would exceed that limit. The complete
source name remains in SWC metadata.
